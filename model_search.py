...
class MixedOp(nn.Module):
  ...
  def forward(self, x, weights, updateType):
    ...
    return sum(result)  #op(x)就是对输入x做一个相应的操作 w1*op1(x)+w2*op2(x)+...+w8*op8(x)
...
class Cell(nn.Module):                       #定义cell
  ...
  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
    ...
    self._steps = steps             #每个Cell的待确定连接状态的节点数
    self._multiplier = multiplier

    self._ops = nn.ModuleList()             #构建operation的modulelist
    for i in range(self._steps):            #遍历4个intermediate nodes构建混合操作
      for j in range(2+i):                  #遍历当前节点的所有前驱节点
        stride = 2 if reduction and j < 2 else 1
        op = MixedOp(C, stride, reduction)   #op是构建两个节点之间的混合
        self._ops.append(op)                    #所有边的混合操作添加到ops,list的len为2+3+4+5=14[[],[],[],[],...[]]

  def forward(self, s0, s1, weights, updateType):     #cell中的计算过程，前向传播自动调用
    s0 = self.preprocess0(s0)
    s1 = self.preprocess1(s1)

    states = [s0, s1]                         #当前节点的前驱节点
    offset = 0
    for i in range(self._steps):               #遍历每个节点，得到每个节点的output
      s = sum(self._ops[offset+j](h, weights[offset+j], updateType) for j, h in enumerate(states))#s为当前节点i的output，在ops找到i对应的操作，然后对i的所有前驱节点做相应的操作（调用了MixedOp的forward），然后把结果相加
      offset += len(states)
...
class Network(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, criterion, greedy=0, l2=0, steps=4, multiplier=4, stem_multiplier=3):  #l2初为0，greedy初为0
